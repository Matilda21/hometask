...
public class FragmentOne extends Fragment implements RecyclerViewAdapter.ItemClickListener {
    Button button;
    ...
    public void onItemClick(View view, int position) {
        if (position % 2 == 0)
            ((MainActivity) getActivity()).putFragmentTwo(adapter.getItem(position),
                    adapter.redColor);
        else
            ((MainActivity) getActivity()).putFragmentTwo(adapter.getItem(position),
                    adapter.blueColor);
